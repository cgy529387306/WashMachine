...
public class DeviceHelper {
    ...
    //保存文件的路径
    private static final String CACHE_IMAGE_DIR = "mb/cache/devices";
    //保存的文件 采用隐藏文件的形式进行保存
...
//        return info.getMacAddress();
    ...
    private static File getDevicesDir(Context context) {
        ...
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            File copyDir = new File(Environment.getExternalStorageDirectory(), CACHE_IMAGE_DIR);
            if (!copyDir.exists()) {
                copyDir.mkdirs();
            }
            mCropFile = new File(copyDir, DEVICES_FILE_NAME); // 用当前时间给取得的图片命名
        } else {
            File copyDir = new File(context.getFilesDir(), CACHE_IMAGE_DIR);
            if (!copyDir.exists()) {
                copyDir.mkdirs();
            }
            mCropFile = new File(copyDir, DEVICES_FILE_NAME);
        }
        return mCropFile;
    }
}
